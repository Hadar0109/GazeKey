...
- [ ] T057 [US5] **USER GATE** — **deferred** 2026-08-24: do **not** record or evaluate `hadar` as an accuracy metric until after integration, when the operator chooses 32M-image GazeFollower eval or key-size/hit-area work. Record: `runs/_feature005/T057_hadar_deferred.md`. Must not override SC-006 / unreachable-control failures later.
...
- [X] T061 [P] [US5] Optional second-user smoke test **explicit skip** 2026-08-24: `runs/_feature005/T061_second_user_skip.md` (FR-035 / SC-019). Restated in T062 `integrated_acceptance.md`. Does **not** release the cleanup hold.
- [X] T062 [US5] Wrote `runs/_feature005/integrated_acceptance.md` with verdict **incomplete / not PASS** (SC-001 third session open; T057 `hadar` deferred). SC-012 cites Stage E T042–T047 **and** T045 live recalibration PASS (`runs/005-t045-recalibrate-20260824/summary.md`). Replay incompleteness stated. Feature 004 T060 formulas are not the cleanup trigger.
- [ ] T063 [US5] **Cleanup hold remains**. T062 is not PASS. T056 and T057 are still open. Do **not** start T064. If later fail/inconclusive, keep the hold, leave legacy code unused, rollback via Git only.
