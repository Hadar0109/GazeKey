...
**Architecture pivot (2026-08-20):** T020 A/B/C investigations are complete
(A **REVERT**; B **INCONCLUSIVE / not keepable and reverted**; C **REVERT**).
Product code and tests remain at `861a89c`. The remaining Feature 004 A–F
task sequence is **paused, not completed**. Historical tasks and runs are
not deleted, renumbered, or rewritten. See
...
- [x] T020 [US2] **Run this first among T019–T022.** Investigate left/right eye-local **basis semantics** in `gazekey/features/extractor.py` (`_eye_uv_one_eye`) vs `gazekey/tracking/` eye-corner order — **`v` as well as `u`**. Evidence: `pca_vR` tracks screen **X** in all 11 sessions (`r(X, vR)` −0.78…−0.97) and `corr(uR, vR)` is +0.78…+0.94, i.e. the right eye's vertical axis is mixed with its horizontal one. That is the highest-value accuracy candidate under a stabilized head, and it also corrupts `avg_v`. If implicated, **one** semantics fix; recalibrate with the support; eval vs A/B (and the T060 reference); keep/revert/inconclusive. Requires T027 (done — evaluation under the product condition is now reachable). Do **not** combine with T027 or T019
  ...
  - **CLOSEOUT 2026-08-20 (investigations complete; product at `861a89c`).** A **REVERT**. B **INCONCLUSIVE / not keepable and reverted** (locked rule: blocked-before-eval = inconclusive; not keepable because X→`v` leak was not removed and 0.15 was not lowered). C **REVERT** (Candidate C product/test code was not kept). Remaining Feature 004 A–F tasks after T027/T060/T020 are **paused, not completed**. `runs/_feature004/architecture_pivot_closeout.md`
...
- **2026-08-20 architecture pivot:** remaining A–F tasks after T027/T060/T020
  are paused, not completed. See
  `runs/_feature004/architecture_pivot_closeout.md`
