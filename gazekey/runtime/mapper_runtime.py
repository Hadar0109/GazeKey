"""Post-calibration mapper fit and runtime predict/clamp."""
...
from gazekey.calibration.quality import assess_fullscreen_feasibility, evaluate_calibration_quality
...
class MapperRuntime:
    """Owns fitted mapper and shared predict/clamp helpers."""
    ...
    def screen_rect_for_fit(self) -> tuple[float, float, float, float]:
        ...
        if clip_rect is not None:
            h._log_verbose(f"[calib] mapper clip bounds: keyboard region {clip_rect}")
            return clip_rect
    ...
    def complete_calibration_fit(
        ...
        *,
        ...
        h = self._host
        h._log_verbose(f"[calib] fitting mapper with {len(samples)} target-mean samples")

        ridge_fit = fit_calibration_mapper(
            samples=samples,
            targets=h._calibration_session.targets,
            calibration_mode=str(h._calib_mode),
        ...
        h._log_verbose(
            f"[calib] mapper fit: success={ridge_fit.success} rms_px={ridge_fit.rms_px} "
        ...
        self._log_ridge_summary(h, ridge_fit, ridge_alpha, loocv_rms)

        selected_type = getattr(ridge_fit.model, "mapper_type", "unknown")
        h._log_verbose(f"[calib] active_mapper={selected_type}")
        ...
        try:
            ...
            h._log_verbose(f"[calib] feasibility assessment failed: {e}")
    ...
    def _ridge_loocv_diagnostics(host, ridge_fit) -> tuple[float, Optional[list], Optional[list], Optional[float]]:
        ...
        try:
            ...
            host._log_verbose(f"[calib] ridge LOOCV compute failed: {e}")
        return ridge_alpha, ridge_loocv_detail, ridge_per_target_err, ridge_loocv_rms

    @staticmethod
    def _log_ridge_summary(host, ridge_fit, ridge_alpha, loocv_rms) -> None:
        try:
        ...
        host._log_verbose(
            f"[calib] ridge diag: feature_count={feature_count} sample_count={sample_count} "
    ...
    def clamp_xy(self, x: float, y: float) -> Tuple[float, float]:
        ...
        if bounds is not None:
            x0, y0, x1, y1 = bounds
            return float(max(x0, min(x1, x))), float(max(y0, min(y1, y)))
